...
real Sigma=0;//grad-div stabilization parameter
   ...
   for(mrit=mri;mrit<=mrf;mrit++){ 
      ...
      if(mr==4) {Ni=N0;Nf=N4;}
          ...
          for(Rait=Rai;Rait<=Raf;Rait++){
             ...
             string E ="P2-P1-P2",S="NC-2level-polynomial func";
             string R;
             ...
             ofstream ffcout("coupled-Pr="+Pr+"-Ra="+Ra+"-kap="+kap+"-"+E+"(Sigma="+Sigma+").txt");

   ffcout<<S+".edp results\n";
   ffcout<<" coupled two-grid method for NC equations\n";   
...
//compute the successious error                
    ...
    fespace Uh01(Th1,P2); Uh01 eh11,eh12,vh11,vh12,u11=u1,u12=u2; // the velocity space
    fespace Ph01(Th1,P1); Ph01 rh1,qh1,p1=p;     // the pressure space
    fespace Teh01(Th1,P2); Teh01 eT1, phi1, TT1=T;     // the pressure space

    problem NCOssen([eh11,eh12,rh1, eT1],[vh11,vh12,qh1,phi1]) =
        int2d(Th1)(Pr*(dx(eh11)*dx(vh11)+dy(eh11)*dy(vh11)+dx(eh12)*dx(vh12)+dy(eh12)*dy(vh12))//a(U,V)
        ...
        -int2d(Th1)(f1*vh11+f2*vh12+g*phi1)    //right hand side
        +on(1,2,3,4,eh11=0,eh12=0, eT1=0);   //boundary condition
